...
def test_release_reservation_frees_budget(tmp_path: Path):
    s = _settings(max_daily_notional=500_000)
    risk = RiskEngine(s, tmp_path, DailyPnLTracker(tmp_path / "pnl.jsonl", timezone_name="UTC"))
    ok, _ = risk.try_reserve_notional(400_000, reservation_id="X")
    assert ok
    risk.release_reservation("X")
    assert risk.reserved_notional() == 0
    ok2, _ = risk.try_reserve_notional(400_000, reservation_id="Y")
    assert ok2


def test_governor_blocks_duplicate(tmp_path: Path):
