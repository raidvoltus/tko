        ...
        *,
        ...
        if lookup.status == OrderLookupStatus.NOT_FOUND:
            ...
            if intent.attempts >= self.max_unknown_checks:
                # S5-B2 / INV-61: escalate to GOVERNOR_AUTONOMOUS which REMAINS in
                # BLOCKS_DUPLICATE. NOT_FOUND after N lookups is NOT cryptographic proof
                # the exchange never accepted the order — keep symbol/side blocked until
                # authoritative account-level resolution (operator or balance proof).
